...
# Processing runtime
#
# ******* STILL EXISTING NOISE, NEED TO CLEAN THE SAMPLE THEN RECHECK
...
filename_output = "D:\Welding_image_process\data\gcode_" + timenow.strftime("%d%m%y")+"_"+timenow.strftime("%H%M%S")+".txt"

file_output= open(filename_output,"w+")
file_output.close() 

# Define the codec and create VideoWriter object
filename_video = "D:\\Welding_image_process\\img_saved\\video_" + timenow.strftime("%d%m%y")+"_"+timenow.strftime("%H%M%S")+".avi"
...
def state_0(): 
    #!!
    #   This function is used for Homing and Saving video sample
    #!!
    appMach3 = Application(backend='uia').connect(title = "Mach3 CNC Controller ")


    #First step - Load Recent File
    loadRF = appMach3.Mach3CNCController.child_window(title="Recent File", auto_id="8349", control_type="Button").wrapper_object()
    loadRF.click()
    time.sleep(0.1)

    #Click 'OK'
    okBTN = appMach3.Mach3CNCController.child_window(title="OK", auto_id="1", control_type="Button").wrapper_object()
    okBTN.click()
    time.sleep(0.1)

    #Edit GCode
    okBTN = appMach3.Mach3CNCController.child_window(title="Edit G-Code", auto_id="8586", control_type="Button").wrapper_object()
    okBTN.click()
    time.sleep(0.1)
...
######################################
# STATE 1
# This function is used to save video sample
# The video sample is named by filename_video
#
######################################
...
######################################
# STATE 2 -> DETECTING, RECOGNIZING AND CREATING GCODE
######################################
...
def state_2():
    ...
    #HoughLine
    lines = cv.HoughLines(skeleton_img,1, np.pi / 180, hough_threshold, None, 0, 0)
    ...
    if lines is not None:
        for i in range(0, len(lines)):
            ...
            if ((pt1[0] < 10000 and pt1[0] > -20000) and    #pt1[0] (X_1) must be in range (-20000;100000)
                (pt2[0] < 20000 and pt2[0] > -20000) and    #pt2[0] (X_2) must be in range (-20000;200000)
                (pt1[0] != 0 and pt2[0] != 0) and           #pt1[0] (X_1) and pt2[0] (X_2) cannot be 0
        ...
        if (num_line != 0):
            ...
            cv.line(img_copy, (ave_top_point,0), (ave_bottom_point,640), (0,0,255), 1)                            
...
#######
                    str_gcode = "G1 X" + str(round(x_dis,4)) + " Y" + str(y_dis) + " F300"   

                    line_prepend(str_gcode) #Add the new str_gcode line above the previous one 
